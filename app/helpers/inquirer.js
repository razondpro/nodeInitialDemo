...
async function showStartedTasks(taskController, user) {
  ...
  while (!exit) {
    ...
    if (menuOption.menu === "Back") {
      ...
      const task = tasksByUser[menuOption.menu.charAt(0) - 1];
      const actionMenu = {
        type: "list",
        name: "action",
        message: `Selected task: ${task.title}`,
        choices: ["View details", "Delete task", "Update status", "Back"],
      };

      const actionOption = await inquirer.prompt([actionMenu]);

      switch (actionOption.action) {
        case "View details":
          console.log(task);
          break;
        case "Delete task":
        ...
        case "Update status":
          // POR HACER
          console.log("Update status selected");
          break;
        case "Back":
          // do nothing
          break;
...
function getTaskTitlesMenu(tasks, message, includeActions = false) {
  const choices = tasks.map((task, index) => `${index + 1}. ${task.title}`);
  choices.push("Back");

  if (includeActions) {
    choices.push(" ");
    choices.push("Select an action:");
  }
